...
export default function GroupStatistics({ nights, reviews, users = [] }: GroupStatisticsProps) {
  ...
  return (
    <div className="bg-[#0c0c0c] border border-zinc-900 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl shadow-[0_0_50px_rgba(0,0,0,0.8)]">
      {/* Header */}
      <div className="border-b border-zinc-900 pb-5">
        <span className="text-[10px] font-mono text-amber-500 uppercase tracking-widest flex items-center gap-1 mb-1">
          <Trophy className="w-3.5 h-3.5" /> Club Insights
        </span>
        <h3 className="text-2xl font-serif text-zinc-100 font-bold tracking-tight">Movie Club Statistics</h3>
        <p className="text-xs text-zinc-400 font-mono mt-0.5">Real-time performance metrics and records of our sessions</p>
      </div>

      {/* Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Stat 1: Movies */}
        <div className="bg-[#121212]/40 border border-zinc-900 rounded-2xl p-5 flex items-start gap-4 hover:border-zinc-800/80 transition-colors">
          <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl shrink-0">
            <Film className="w-5 h-5" />
          </div>
          <div className="space-y-1">
            <span className="text-xs text-zinc-500 font-mono uppercase tracking-wider block">Watched</span>
            <div className="text-2xl font-bold font-mono text-zinc-100">{totalWatched}</div>
            <span className="text-[10px] text-zinc-400 block font-sans">Movie sessions completed</span>
          </div>
        </div>
        ...
        <div className="bg-[#121212]/40 border border-zinc-900 rounded-2xl p-5 flex items-start gap-4 hover:border-zinc-800/80 transition-colors">
          ...
          <div className="space-y-1">
            <span className="text-xs text-zinc-500 font-mono uppercase tracking-wider block">Screen Time</span>
            <div className="text-2xl font-bold font-mono text-zinc-100">{totalHours}h</div>
            <span className="text-[10px] text-zinc-400 block font-sans">Based on film runtimes</span>
          </div>
        </div>
        ...
        <div className="bg-[#121212]/40 border border-zinc-900 rounded-2xl p-5 flex items-start gap-4 hover:border-zinc-800/80 transition-colors">
          ...
          <div className="space-y-1">
            <span className="text-xs text-zinc-500 font-mono uppercase tracking-wider block">Avg Rating</span>
            <div className="text-2xl font-bold font-mono text-zinc-100">
              {avgRating} <span className="text-xs text-zinc-500">/ 5</span>
            </div>
            <span className="text-[10px] text-zinc-400 block font-sans">Calculated across {totalReviews} reviews</span>
          </div>
        </div>

        {/* Stat 4: Favourite Genre */}
        <div className="bg-[#121212]/40 border border-zinc-900 rounded-2xl p-5 flex items-start gap-4 hover:border-zinc-800/80 transition-colors min-w-0">
          <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl shrink-0">
            <Flame className="w-5 h-5" />
          </div>
          <div className="space-y-1 min-w-0">
            <span className="text-xs text-zinc-500 font-mono uppercase tracking-wider block">Top Genre</span>
            <div className="text-base font-bold text-zinc-100 break-words leading-tight" title={favoriteGenre}>
              {favoriteGenre}
            </div>
      ...
      <div className="bg-zinc-950/40 border border-zinc-900 rounded-2xl p-5 flex flex-col md:flex-row items-center justify-between gap-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-sm font-bold text-zinc-200 flex items-center gap-2">
              Best Reviewer Award
              <span className="inline-flex items-center gap-0.5 px-2 py-0.5 text-[9px] bg-amber-500/10 border border-amber-500/30 text-amber-500 font-mono rounded-full">
        ...
        <div className="shrink-0 flex items-center gap-3">
          {maxStarredCount > 0 && bestReviewers.length > 0 ? (
            <div className="flex items-center gap-3 bg-zinc-900/60 border border-zinc-800/80 px-4 py-2.5 rounded-xl">
              <div className="flex -space-x-2">
                {bestReviewers.map((reviewer) => {
                  const avatar = getReviewerAvatar(reviewer);
                  return (
                    <div key={reviewer} className="w-8 h-8 rounded-full overflow-hidden border border-zinc-950 bg-zinc-900 flex items-center justify-center">
                      {avatar ? (
                        <img src={avatar} alt={reviewer} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-xs font-bold text-amber-500 uppercase">{reviewer.substring(0, 2)}</span>
                      )}
                    </div>
                  );
                })}
            ...
            <div className="text-xs text-zinc-500 font-mono italic bg-zinc-900/30 border border-dashed border-zinc-900 px-4 py-2.5 rounded-xl">
